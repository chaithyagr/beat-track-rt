...
class BeatTrackAll(object):
    ...
    def close(self):
        ...
        while (self.t1.isAlive()):  # wait for all threads to close
            time.sleep(.1)
    ...
    def stream_thread_new(self):
        self.t1 = threading.Thread(target=self.stream_readchunk)
        self.t1.start()
    ...
    def stream_thread_start_beatseq(self):
        self.t3 = threading.Thread(target=self.printbeat)
        self.t3.start()

    def stream_thread_start_song(self):
        self.t4 = threading.Thread(target=self.playsong)
        self.t4.start()
    ...
    def playsong(self):
        ...
        try:
            ...
            pass
        f4 = open('Beats/'+'Beat'+self.song_no+'.txt', 'w')
    ...
    def getbpm(self):
        ...
        wv = (
            np.divide(n, np.power(self.rayparam, 2)) * np.exp(
...
if __name__=="__main__":
    dir=input('Please enter the Directory to run : ')
    dir=str(dir)+'*.wav'
    files=glob.glob(str(dir))
    ctr=1
    for song in files:
        ear = BeatTrackAll(song,ctr)
        ear.stream_start()
        print(ctr)
